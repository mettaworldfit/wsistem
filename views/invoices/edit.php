        ...
        <?php while ($element = $detalle_factura->fetch_object()): ?>
            ...
            <tbody id="rows">
                <tr>
                    ...
                    <td><?= number_format($element->cantidad_total * $element->impuesto, 2) ?> - (<?= $element->valor ?>%)
                    </td>
                    <td><?= number_format($element->descuento, 2) ?></td>
                    <td><?= number_format(($element->cantidad_total * $element->precio) + ($element->cantidad_total * $element->impuesto) - $element->descuento, 2) ?>
                    </td>
                    <td>
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <div class="input-div">
                                    ...
                                    <select class="form-custom-icon search" name="" id="customer" requireds>
                                        <option value="<?= $info->cliente_id ?>" selected>
                                            <?= ucwords($info->nombre_cliente) ?>     <?= ucwords($info->apellidos_cliente) ?>
                                        </option>
