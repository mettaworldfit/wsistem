...
// ===== Datos Gastos =====
    ...
    SELECT 
        CONCAT('G-00', x.orden_id) AS factura,
        m.descripcion AS descripcion,
        concat(p.nombre_proveedor,' ', IFNULL(p.apellidos, '')) as proveedor,
        dg.cantidad,
        dg.precio,
        '0' AS descuento,
        x.fecha, '-' as tipo_pago
    FROM gastos x
    INNER JOIN ordenes_gastos og ON x.orden_id = og.orden_id
    ...
    INNER JOIN facturas_proveedores x ON x.orden_id = dc.orden_id
    INNER JOIN metodos_de_pagos mp ON mp.metodo_pago_id = x.metodo_pago_id
