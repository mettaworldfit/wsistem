function editProduct() {
    ...
    sendAjaxRequest({
        url: "src/modules/products/products.repository.php", // Archivo PHP que procesa la acción
        data: {
            name,
            product_id: $("#product_id").val(),
            product_code: $("#product_code").val(),
            price_out: priceOut,
            price_in: $("#inputPrice_in").val() || 0,
            quantity,
            min_quantity: $("#input_min_quantity").val() || 0,
            provider: $("#provider").val() || 0,
            tax: $("#tax").val() || 0,
            brand: $("#brand").val() || 0,
            offer: $("#offer").val() || 0,
            category: $("#category").val() || 0,
            position: $("#position").val() || 0,
            warehouse: $("#warehouse").val() || 0,
...
$(document).ready(function () {
    ...
    function createProduct() {
        sendAjaxRequest({
            ...
            data: {
                ...
                min_quantity: $("#min_quantity").val() || 1,
    ...
    * MANEJO DE IMAGEN
    ...
    function uploadImage(productId) {
        ...
        if (file) {
            ...
            $.ajax({
                url: SITE_URL + 'src/modules/products/products.repository.php',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
    ...
    * GENERAR CODIGO DE BARRA
    ...
    $('#generate_code').on('click', function (e) {
        ...
        const url = SITE_URL + "src/services/labels/label.php" +
            "?etiqueta_id=1" +
            "&name=" + product_name +
            "&code=" + product_code +
            "&price=" + product_price;
