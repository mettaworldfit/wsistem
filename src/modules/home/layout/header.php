...
<head>
  ...
  <script src="<?= base_url ?>/scripts.js?v=<?= APP_VERSION ?>"></script>
  ...
  <?php if (isset($_SESSION['admin']) || isset($_SESSION['identity'])) { ?>
    ...
    <link rel="stylesheet" href="<?= base_url ?>/style.css?v=<?= APP_VERSION ?>" type="text/css">

  <?php } else { ?>

    <link rel="stylesheet" href="<?= base_url ?>/src/modules/users/login.css?v=<?= APP_VERSION ?>" type="text/css">

  <?php } ?>

  <!-- Lazy loading -->
  <?php require_once dirname(__DIR__, 3) . "/functions/lazy_load.php"; ?>
  ...
  <?php if (isset($_SESSION['admin']) || isset($_SESSION['identity'])): ?>
    ...
    <div class="main wrap">
      <main>
        ...
        <?php require_once __DIR__ . '/includes/modal-global.php'; ?> <!-- Modals goblales -->
