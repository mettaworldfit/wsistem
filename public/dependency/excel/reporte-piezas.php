...
$query = "SELECT *, p.pieza_id as idpieza FROM piezas p 
        ...
        LEFT JOIN piezas_con_posiciones pps ON p.pieza_id = pps.pieza_id
...
while($result = $datos->fetch_object()) {
    ...
    $sheet->setCellValue('A' .$i, ucwords($result->nombre_pieza));
    ...
    $sheet->setCellValue('E' .$i, ucwords($result->nombre_marca));
    $sheet->setCellValue('F' .$i, ucwords($result->nombre_categoria));
...
header('Content-Disposition: attachment;filename="reporte-piezas.xlsx"');
