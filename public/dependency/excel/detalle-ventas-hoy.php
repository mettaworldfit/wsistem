 ...
 * Stylos
 ...
 $query = "SELECT * FROM (
     ...
     inner join productos p on p.producto_id = dp.producto_id where f.fecha = curdate()
     ...
     inner join servicios s on s.servicio_id = ds.servicio_id where f.fecha = curdate()
     ...
     inner join piezas pz on pz.pieza_id = dp.pieza_id where f.fecha = curdate()
     ...
     inner join servicios s on s.servicio_id = dp.servicio_id where f.fecha = curdate()
 ...
 while ($result = $datos->fetch_object()) {
     ...
     if ($total > 0) {
         ...
         $sheet->setCellValue('B' . $i, ucwords($result->descripcion));
...
header('Content-Disposition: attachment;filename="Reporte-Hoy-'.date("Y-m-d").'.xlsx"');
